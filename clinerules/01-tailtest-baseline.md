...
- **Manual mode (default):** the user invokes the test cycle explicitly via `/tailtest-test <file>` or natural language ("test the file I just edited"). When invoked, run the full cycle below.

In both modes, prefer calling the structured MCP tools (`tailtest_scenario_plan`, `tailtest_classify_failures`, `tailtest_pick_template`) over reasoning over rule text alone. The MCP tools provide deterministic results that the agent's prose cannot.
...
## Step 1: When the user has just edited a file (auto mode) or invoked `/tailtest-test` (manual mode)

Run the full test cycle for the file in question.
...
1. Identify the file edited or named in the slash command
2. Read `.tailtest/session.json` for runner config and session state
3. Skip the file if it matches the filter rules in Step 2
...
5. Generate scenarios for the file (Step 3)
6. Write test file and execute (Steps 4-5)
7. Report failures -- stay silent if all pass (Step 6)
8. Update `generated_tests` in `.tailtest/session.json`

If multiple files were edited in the same turn (auto mode), treat them as one cohesive unit. If you wrote a service, a model, and a controller this turn, generate one scenario set for what the system does -- not three disconnected sets.

Write **ONE test file** for the entire batch, named after the primary source file (the entry point, controller, or most feature-rich file). Do not create separate test files for models, helpers, or type-only modules -- cover them through the primary file's tests. After writing the test, record every source file in the batch in `.tailtest/session.json`:
```json
...
- `.clinerules/` (the rule pack -- never test rule files)
- `memory-bank/` (Cline's persistent context -- never test memory files)

**By file name:**
- Contains `test_`, `.test.`, `.spec.`, `_spec.`, `_test.`, `Test.`, `Tests.` -- it is a test file
- Contains `_hunt.` (e.g. `test_billing_hunt.py`) -- it is a hunt test file from `/tailtest hunt`
...
**Use Plan mode for this step when in manual mode.** Cline's Plan mode is purpose-built for plan-before-write. Output the SCENARIO PLAN inside Plan mode, let the user review, then switch to Act mode for Steps 4-5.

In auto mode, output the SCENARIO PLAN inline before any Edit or Write tool calls.
...
**Prefer the MCP tool.** When the `tailtest-mcp` server is available, call `tailtest_scenario_plan(file_path)` to get a structured plan instead of reasoning ad-hoc. The MCP tool reads the file, infers language and framework, applies R15, and returns a SCENARIO PLAN matching the format above.
...
**Framework-specific baseline scenarios** -- when `session.json` contains a framework tag, always include these in addition to the language baselines. The MCP tool `tailtest_pick_template(file_path)` returns the appropriate template for the framework if available.
...
If the existing test file for this source already exists, read it first to understand structure / helpers / conventions. Add new scenarios or update tests for what changed -- do not replace the entire file and do not regenerate scenarios that already pass.

If no test file exists yet, create it. Create the test directory if it does not exist.
...
**Prefer the MCP tool.** When the `tailtest-mcp` server is available, call `tailtest_classify_failures(runner_output)` to get structured classification. The tool returns `[{type, file, line, reason}]` arrays that you can render directly.
...
**When the existing test file should be re-run only:**
...
## /tailtest-summary command

When the user types `/tailtest-summary` (or a natural variant like "tailtest summary", "what did tailtest do", "what did you test"):
...
After outputting the summary to the conversation, also write the same content to the file at `report_path` in session.json (the `.tailtest/reports/` directory -- create it if it does not exist). If `report_path` is not set in session.json, skip writing.
...
`paused` is not persisted across sessions. New sessions always initialise it to `false`.

Do not emit this output automatically. Only respond when the user explicitly types one of these commands.

---

## /tailtest-test command (manual mode trigger)

When the user types `/tailtest-test <file>` (or a variant like "tailtest <file>", "run tailtest on <file>", "test the file I just edited"):
...
This is the only way to trigger generation for a file that tailtest would normally skip (legacy file with no existing tests, or any file the user wants explicitly covered). It is also the manual-mode counterpart to auto mode's automatic post-edit firing.

---

## /tailtest hunt command (one-shot adversarial pass)

See the dedicated workflow file `.clinerules/workflows/tailtest-hunt.md`. The hunt slash command bypasses depth and runs an 8-12 scenario adversarial pass on a specific file, writing to a separate `tests/test_<basename>_hunt.py` file so the hunt does not contaminate the main test suite.

---

## /tailtest-mode auto / /tailtest-mode manual

When the user types `/tailtest-mode auto` or `/tailtest-mode manual`:
1. Read `.tailtest/session.json`
2. Set `"mode": "auto"` or `"mode": "manual"`
3. Write back
4. Respond:
   - For `auto`: "tailtest is now in auto mode. After every code edit, the test cycle will run automatically. This requires Cline's auto-approve to be enabled for 'Edit files (workspace)', 'Execute safe commands', and 'Use MCP servers'."
   - For `manual`: "tailtest is now in manual mode. Run the test cycle by typing `/tailtest-test <file>` after edits."

Default for new installs is `manual` (less surprising for first-run users).
